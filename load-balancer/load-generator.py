...
async def send_request(session, replica_url, prompt_len, req_id):
    payload = {
    ...
    try:
        async with session.post(replica_url, json=payload) as resp:
            await resp.text()
            print(f"req={req_id} replica={replica_url[-4:]} prompt_len={prompt_len} status={resp.status}")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while asyncio.get_event_loop().time() - start < duration_seconds:
            ...
            for _ in range(concurrency):
                ...
                tasks.append(send_request(session, replica, prompt_len, req_id))
                req_id += 1
